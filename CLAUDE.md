...
- `.devcontainer/on-create.sh` runs once on codespace creation
...
- `docroot/sites/settings/codespaces.settings.php` contains Drupal settings overrides
...
- Update `.devcontainer/on-create.sh` if the site installation command changes (currently `drush sws:multisite:install --site=default`)
