...
class CircleCiCommands extends BltTasks {
  ...
  protected function runUnitTestsWithCoverage() {
    ...
    $tasks[] = $this->taskExecStack()->dir($docroot)
      ->exec('../vendor/bin/phpunit -c core --debug --verbose --coverage-clover ../artifacts/coverage/clover.xml --coverage-html ../artifacts/coverage/html --filter "/(Unit|Kernel)/" ' . static::TEST_DIR);
    return $tasks;
  }
