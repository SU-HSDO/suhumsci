...
class CircleCiCommand extends BltTasks {
  ...
  protected function syncAcquia($site = 'swshumsci') {
    ...
    // of the db dump.
    $tasks[] = $this->taskDrush()->rawArg("@$site.prod sql-connect");
    $tasks[] = $this->taskDrush()->rawArg("@$site.prod sql-dump > dump.sql");
    ...
    $tasks[] = $this->taskDrush()->rawArg('@self sql-cli < clean_dump.sql');
    return $tasks;
  }
