...
class Date extends FieldCloneBase {
  ...
  protected function incrementDateValue($value, array $increment_config = []) {
    ...
    $original_daylight_savings = date('I', $new_value->getTimestamp());
    ...
    // savings time.
    if (date('I', $new_value->getTimestamp()) != $original_daylight_savings) {
      // Accommodate both going into and out of daylight savings time.
      $interval = $original_daylight_savings ? '1 hour' : '-1 hour';
