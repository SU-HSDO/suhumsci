...
Sass Mixins allow us to write clean, consistent code by defining styles that can be re-used throughout the project. Mixins are found under the `/src/scss/partials` directory.
...
| @mixin generate-css-vars($map-id, $map) { } | Generates CSS variables from a Sass map. Supports nested maps. Used only in base theme files. | <code>:root { @include generate-css-vars('traditional-global', $traditional-global-colors); }</code>
