...
`.devcontainer/devcontainer.json` specifies the container image, port forwarding, and the `onCreateCommand`/`postAttachCommand` hooks that drive initialization.
...
- `.devcontainer/on-create.sh` runs once, at codespace creation. It installs `nvm` (letting `hooks/common/theme/theme-get-command.sh`'s own detection install the exact Node version from `.nvmrc`), installs Composer dependencies, builds theme CSS/JS (not committed to the repo), generates multisite settings, installs the default site, configures Apache, and rebuilds the cache. It never runs again, including on resume, so it does not fetch new commits, run database updates, or import configuration.
...
- **CSS or JS 404s or looks broken or unstyled:** Theme assets are not committed to the repo and must be built explicitly. Confirm `.devcontainer/on-create.sh` still installs `nvm` and sources it before `composer build-theme` runs, and still runs `drush @default.local cr` and the file-permission fix for `docroot/sites/default/files` as its last steps.
