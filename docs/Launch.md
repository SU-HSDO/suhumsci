...
1. **Activate the new version**
   ...
   * Communicate the activation in the internal WAF / CDN channel
   * Once staging is confirmed, activate on production
   * Confirm successful activation

### Handle Domain Pointing Mismatch
...
> **Important:** PHP syntax errors in `sites.php` will cause fatal errors across all sites on the stack. Write and test the code locally first.

* SSH into the server via drush:
  ```bash
  drush @<SITE_ALIAS> ssh
  ```
...
1. Add it to the custom URL pointing section of `docroot/sites/sites.php`
1. Create a PR to commit the change permanently
...
* If the redirect doesn't work, try a cache-busting query string: `mysite.stanford.edu?foo=bar`
* If still failing, clear Drupal and Varnish caches

### Review Launch Notes

Review the launch notes or checklist for any site-specific requirements or caveats before proceeding.
...
> **Note:** Redirect imports were heavy during the D7-to-D8 switchover. This section is mostly relevant only for legacy migrations; most current launches do not require redirect imports.

#### Download the Spreadsheet

* Obtain the Google Sheet URL redirects from the launch notes
* Download as CSV
...
Navigate to the prod site (`<SITE_ALIAS>-prod.stanford.edu` or the live URL if already switched):
...
> **Note:** If an import fails because a page was renamed, investigate and fix if possible; otherwise note it and report in the launch notes.
...
1. Set the verification tag via drush:
   ```bash
   drush @<SITE_ALIAS>.prod cset metatag.metatag_defaults.global tags.google_site_verification <TAG_VALUE> -y && drush @<SITE_ALIAS>.prod cr
   ```

1. Rebuild Drupal cache and verify in Google Console (retry with cache clear if needed)

1. Add sitemap to Google:
   ...
   * If links are broken, rebuild the sitemap and recheck:
     ```bash
     drush @<SITE_ALIAS>.prod xmlsitemap:rebuild
     ```
...
```bash
drush @<SITE_ALIAS>.prod cset domain_301_redirect.settings enabled 0 -y && \
drush @<SITE_ALIAS>.prod en nobots -y && \
drush @<SITE_ALIAS>.prod sset nobots 1 -y && \
drush @<SITE_ALIAS>.prod cr
```

Or run commands individually:

```bash
drush @<SITE_ALIAS>.prod cset domain_301_redirect.settings enabled 0
drush @<SITE_ALIAS>.prod en nobots
drush @<SITE_ALIAS>.prod sset nobots 1
```

### Notify H&S

Update the launch notes and notify H&S through the appropriate channel:

> The **`<SITENAME>`** site has launched successfully: `<SITE_URL>`
> - All redirects imported successfully (or note any issues)
> - The previous site has been set to nobots: `<OLD_SITE_URL>`
